...
const DetailForm = (() => {
  ...
  function _renderBlock(b, bi) {
    ...
    if (b.type==='steps') {
      return `<div class="dblock">
        ...
        <button class="add-step-btn" onclick="DetailForm._addItem(${bi})">+ Adım</button>
      </div>`;
    }
    if (b.type==='box') {
      return `<div class="dblock">
  ...
  function _renderMetaSection() {
    ...
    return `
      ...
      <div class="form-section">
        ...
        <div id="df-tags">${_renderTags()}</div>
  ...
  function _toggleTag(t){ const i=_d.ingTags.indexOf(t); if(i>=0)_d.ingTags.splice(i,1); else _d.ingTags.push(t); _patchTags(); _patchTagBrowser(); }
  function _removeTag(i){ _d.ingTags.splice(i,1); _patchTags(); _patchTagBrowser(); }
  ...
  return {
    ...
    _toggleCat, _toggleTag, _removeTag, _toggleTagBrowser,
