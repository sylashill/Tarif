...
const Form = (() => {
  ...
  // Tüm form alanlarının kalıcı state'i (sayfa değişince DOM silinir, burası kalır)
  let _formData = {
    name:'', story:'', ingredients:'', notes:'',
  ...
  function _reset() {
    ...
    _formPage      = 0;
    _formData = { name:'', story:'', ingredients:'', notes:'', duration:'', servings:2, difficulty:'Orta', calories:'', sourceLink:'', stepsText:'' };
  }
  ...
  function _renderPage0() {
    const name  = _formData.name;
    const story = _formData.story;
    const ingVal= _formData.ingredients;
    const notes = _formData.notes;
    const diff  = _formData.difficulty || 'Orta';
    const dur   = _formData.duration || '';
    const serv  = _formData.servings || 2;
    const cals  = _formData.calories || '';
    const link  = _formData.sourceLink || '';
        ...
        <div id="steps-editor" style="margin-top:10px">
          ...
          ` : `
            <textarea class="form-textarea" id="f-steps-single" rows="8"
              ...
              style="white-space:pre-wrap">${_esc(_formData.stepsText||'')}</textarea>
          `}
        </div>
      </div>
  ...
  // Steps — her işlem öncesi DOM'u kaydet
  function _setStepsMode(mode) { _collectCurrentPage(); _stepsMode = mode; _renderPage(); }
  function _addStep()          { _collectCurrentPage(); _stepRows.push(''); _renderPage(); }
  function _removeLastStep()   { _collectCurrentPage(); if (_stepRows.length > 1) { _stepRows.pop(); _renderPage(); } }
  function _removeStep(i)      { _collectCurrentPage(); _stepRows.splice(i,1); _renderPage(); }
  ...
  function _collectCurrentPage() {
    if (_formPage === 0) {
      // Tüm sayfa 0 alanlarını _formData'ya kaydet
      const g = id => document.getElementById(id);
      if (g('f-name'))        _formData.name        = g('f-name').value;
      if (g('f-story'))       _formData.story       = g('f-story').value;
      if (g('f-ingredients')) _formData.ingredients = g('f-ingredients').value;
      if (g('f-notes'))       _formData.notes       = g('f-notes').value;
      if (g('f-duration'))    _formData.duration    = g('f-duration').value;
      if (g('f-servings'))    _formData.servings    = g('f-servings').value;
      if (g('f-difficulty'))  _formData.difficulty  = g('f-difficulty').value;
      if (g('f-calories'))    _formData.calories    = g('f-calories').value;
      if (g('f-source-link')) _formData.sourceLink  = g('f-source-link').value;
      if (g('f-steps-single'))_formData.stepsText   = g('f-steps-single').value;
      // Adım textarea'larını topla
      document.querySelectorAll('[data-step]').forEach(ta => {
  ...
  // Populate (edit mod) — _formData'yı doldurur, _renderPage0 oradan okur
  function _populateFields(r) {
    const ingVal = (r.ingredients || []).map(i =>
      typeof i === 'object' ? `${i.amount||''} ${i.unit||''} ${i.name||i}`.trim() : i
    ).join(', ');
    _formData.name        = r.name        || '';
    _formData.story       = r.story       || '';
    _formData.ingredients = ingVal;
    _formData.notes       = r.notes       || '';
    _formData.duration    = r.duration    || '';
    _formData.servings    = r.servings    || 2;
    _formData.difficulty  = r.difficulty  || 'Orta';
    _formData.calories    = r.calories    || '';
    _formData.sourceLink  = r.sourceLink  || '';
    _formData.stepsText   = r.stepsText   || '';
    // Sayfayı yeniden render et ki değerler görünsün
    _renderPage();
  }
  ...
  async function save(isDraft = false) {
    ...
    const ingredients = _formData.ingredients.split(',').map(s => s.trim()).filter(Boolean);

    const recipe = {
      id,
      name: name || 'İsimsiz Tarif',
      story:       _formData.story.trim(),
      categories:  _selectedCats.length ? _selectedCats : ['Genel'],
      duration:    _formData.duration.trim(),
      servings:    parseInt(_formData.servings) || 2,
      difficulty:  _formData.difficulty || 'Orta',
      calories:    String(_formData.calories||'').trim(),
      sourceLink:  String(_formData.sourceLink||'').trim(),
      ingredients,
      ingTags:     _confirmedTags,
      stepsMode:   _stepsMode,
      stepsText:   _stepsMode === 'single' ? _formData.stepsText : '',
      stepsArray:  _stepsMode === 'list'   ? _stepRows.filter(s => s.trim()) : [],
      notes:       _formData.notes.trim(),
