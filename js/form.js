...
const Form = (() => {
  ...
  function _renderPage() {
    // Progress bar her sayfa değişiminde güncellenmeli
    _renderProgressBar();
    ...
    if (prevBtn)  prevBtn.style.display  = _formPage > 0 ? 'flex' : 'none';
    if (nextBtn)  nextBtn.style.display  = _formPage < PAGES.length-1 ? 'flex' : 'none';
    if (saveBtn)  saveBtn.style.display  = _formPage === PAGES.length-1 ? 'flex' : 'none';
